...
const headers: Record<headerList, boolean> = {
  charconv: false,
  map: false,
  set: false,
  unordered_map: false,
...
function generateHeader(): string[] {
  const stdheaders = ['cstdint', 'optional', 'string_view'];
  for (const hdr of Object.keys(headers) as headerList[]) {
    if (headers[hdr]) {
      stdheaders.push(hdr);
    }
  }
...
${stdheaders.sort().map((nm) => `#include <${nm}>\n`)}
...
inline constexpr std::optional<${name}> from_string<${name}>(string_view _str) {
  ${typeName} _val;
  auto [ptr, ec] = std::from_chars(_str.data(), _str.data() + _str.size(), _val);
  if (ec != std::errc{}) {
    return std::nullopt;
  }
  return is_valid(_val) ? _val : std::nullopt;
}
